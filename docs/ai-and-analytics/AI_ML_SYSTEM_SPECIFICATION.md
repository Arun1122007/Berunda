...
[//]: # (Document ID: BERUNDA-AI-001 | Version: 1.0 | Status: DRAFT | Classification: CONFIDENTIAL | Owner: Berunda Team | Audience: Developers, Data Scientists | Source: 01_Enterprise_Blueprint Â§8 | Last Verified: 2026-07-17 | Review: Monthly)
...
BriefFacts (NVARCHAR(MAX))
  â†’ Catalyst Function: spaCy en_core_web_lg
  â†’ Custom entity patterns for Indian names, vehicle numbers
  â†’ Post-processing: deduplicate within case, confidence scoring
  â†’ Output: PersonEntityLink, VehicleLink records
```
...
| Vehicle number | â€” | `[A-Z]{2}-\d{2}-[A-Z]{1,2}-\d{4}` | "KA-01-AB-1234" |
| Phone number | â€” | `\d{10}` | "9876543210" |
...
Specified in detail in `docs/data/ENTITY_RESOLUTION_SPECIFICATION.md`.
...
**Features used:**
- `num_prior_cases` â€” count of prior FIRs where PersonEntity appears
- `num_districts` â€” number of distinct districts in prior cases
- `age_group` â€” binned (18-25, 26-35, 36-50, 50+)
- `crime_type_diversity` â€” number of distinct crime heads
- `has_co_accused` â€” whether person appears with co-accused
- `arrest_count` â€” number of arrest records
- `avg_case_severity` â€” average gravity of prior cases
- `recency_days` â€” days since last case appearance
...
User Question (English)
  â†’ Intent Classification (predefined template OR free-text RAG)
  â†’ [Template path] â†’ Parameterized Catalyst Data Store query â†’ Response
  â†’ [RAG path] â†’ Retrieve from int_RAGCorpusChunk (vector similarity)
                â†’ QuickML LLM with chunk context â†’ Grounded answer + citations
```
...
| **Training/Inference Approach** | Inference only â€” pre-trained spaCy `en_core_web_lg`; no custom training for MVP |
...
| **Confidence/Uncertainty** | Weighted composite score 0.00000â€“1.00000; grey zone (0.50â€“0.85) triggers manual review |
| **Human Review Point** | Grey zone (LOW_THRESHOLD 0.50 < score â‰¤ HIGH_THRESHOLD 0.85); reviewed in Investigator Console |
...
| **Output Contract** | Score DECIMAL(6,5) (0.00000â€“1.00000) + Feature importance JSON array |
...
| **Confidence/Uncertainty** | Raw score value 0â€“1; higher values indicate stronger recidivism signal |
