...
[//]: # (Document ID: BERUNDA-SEC-006 | Version: 1.0 | Status: DRAFT | Classification: CONFIDENTIAL | Owner: Berunda Team | Audience: Developers, Governance | Source: 01_Enterprise_Blueprint §12 + SRS security/privacy reqs | Last Verified: 2026-07-17 | Review: Monthly)
...
- GET list endpoints (without specific ID) — too noisy, low value
- Health check endpoint — high frequency, no data exposure
- Static file loads (CSS, JS, images) — no data exposure

## 2. Audit Log Schema

See `docs/05_DATA/CANONICAL_DATA_MODEL.md` Section 3.1 for the full `gov_AuditLog` schema.
...
| Sequential IDs | Auto-increment BIGINT PK — gaps indicate tampering |
...
Step 3: Entity resolution computes similarity scores:
  - Venkatesh ↔ Venkatesha: 0.82 (GREY ZONE)
  - Venkatesh ↔ Venkat: 0.73 (GREY ZONE)
  - Venkatesh ↔ V.: 0.51 (GREY ZONE)
...
This chain is fully auditable — each step is logged in `gov_AuditLog` with timestamps and reviewer identity.
