...
async def seed_lookups(session: AsyncSession):
    ...
    acts = [
        Act(
            ActCode="BNS",
            ActDescription="Bharatiya Nyaya Sanhita 2023",
            ShortName="BNS",
            Active=True,
        ),
        Act(
            ActCode="IT_Act",
            ActDescription="Information Technology Act 2000",
            ShortName="IT Act",
            Active=True,
        ),
        Act(
            ActCode="NDPS",
            ActDescription="Narcotic Drugs and Psychotropic Substances Act",
            ShortName="NDPS",
            Active=True,
        ),
    ...
    sections = [
        ...
        Section(
            ActCode="BNS", SectionCode="109", SectionDescription="Attempt to murder", Active=True
        ),
        Section(
            ActCode="BNS",
            SectionCode="115",
        ...
        Section(
            ActCode="BNS", SectionCode="329", SectionDescription="Criminal trespass", Active=True
        ),
        ...
        Section(
            ActCode="IT_Act",
            SectionCode="67",
            SectionDescription="Publishing obscene material",
            Active=True,
        ),
        Section(
            ActCode="NDPS", SectionCode="20", SectionDescription="Cultivation etc.", Active=True
        ),
        Section(
            ActCode="NDPS", SectionCode="21", SectionDescription="Manufacture etc.", Active=True
        ),
        Section(
            ActCode="Arms_Act", SectionCode="25", SectionDescription="Arms possession", Active=True
        ),
...
async def seed_cases(session: AsyncSession):
    ...
    for i, (accused_name, age, complainant, crime_no) in enumerate(repeat_names):
        ...
        session.add(
            InvOccuranceTime(
                CaseMasterID=cm_id,
                IncidentFromDate=datetime(2026, 1, 1, 20, 0) + timedelta(days=i * 30),
    ...
    for i, (cm_id, crime_no, complainant, brief, lat, lon) in enumerate(vehicle_cases):
        ...
        session.add(
            InvOccuranceTime(
                CaseMasterID=cm_id,
                IncidentFromDate=datetime(2026, 5, 1, 22, 0) + timedelta(days=i * 15),
                Latitude=lat,
                Longitude=lon,
                BriefFacts=brief,
            )
        ...
        session.add(
            Accused(
                AccusedMasterID=10 + i,
                CaseMasterID=cm_id,
                AccusedName=f"Unknown Person {chr(65 + i)}",
                AgeYear=30,
            )
        )
        session.add(
    ...
    for i in range(5):
        ...
        session.add(
            InvOccuranceTime(
                CaseMasterID=cm_id,
                IncidentFromDate=datetime(2026, 2, 15, 19, 30) + timedelta(days=i * 3),
                Latitude=12.97,
                Longitude=77.59,
        ...
        session.add(
            ComplainantDetails(
                ComplainantID=20 + i,
                CaseMasterID=cm_id,
                ComplainantName=f"Hotspot Complainant {i + 1}",
                AgeYear=25 + i * 3,
            )
        )
        session.add(
            Accused(
                AccusedMasterID=20 + i,
                CaseMasterID=cm_id,
                AccusedName=f"Hotspot Suspect {i + 1}",
                AgeYear=28 + i * 2,
            )
        )
    ...
    extra_details = [
        ...
        (20, "KA-09-001", "Kidnapping case", 9, 9),
    ...
    for i, (cm_id, crime_no, brief, major, minor) in enumerate(extra_details):
        ...
        session.add(
            InvOccuranceTime(
                CaseMasterID=cm_id,
                IncidentFromDate=datetime(2026, 4, 1, 18, 0) + timedelta(days=i * 7),
        ...
        session.add(
            ComplainantDetails(
                ComplainantID=30 + i,
                CaseMasterID=cm_id,
                ComplainantName=f"Complainant {cm_id}",
                AgeYear=35,
            )
        )
        session.add(
            Accused(
                AccusedMasterID=30 + i,
                CaseMasterID=cm_id,
                AccusedName=f"Suspect {cm_id}",
                AgeYear=30,
            )
        )
...
async def seed_compute(session: AsyncSession):
    """Pre-compute hotspot layers, anomaly alerts, and risk scores for demo."""

    existing_risk = await session.execute(select(RiskScore).limit(1))
    if existing_risk.scalar_one_or_none():
        print("Computed data already exists; skipping.")
        return

    now = datetime.now(timezone.utc)

    # Risk scores for each person entity
    from src.services.risk_service import RiskService

    risk_svc = RiskService(session)
    entity_result = await session.execute(select(PersonEntity.PersonEntityID))
    entity_ids = [row[0] for row in entity_result]
    for eid in entity_ids:
        score = await risk_svc.compute_risk_score(eid)
        print(f"  Risk score for entity {eid}: {score.Score}")

    # Anomaly alerts for districts with crime spikes
    district_crime_count = await session.execute(
        select(
            CaseMaster.PoliceStationID,
            CaseMaster.CrimeMajorHeadID,
            func.count(CaseMaster.CaseMasterID).label("cnt"),
        ).group_by(CaseMaster.PoliceStationID, CaseMaster.CrimeMajorHeadID)
    )
    rows = district_crime_count.all()
    all_counts = [r.cnt for r in rows]
    mean = sum(all_counts) / len(all_counts) if all_counts else 1.0
    variance = (
        sum((c - mean) ** 2 for c in all_counts) / len(all_counts) if len(all_counts) > 1 else 1.0
    )
    std_dev = math.sqrt(variance) if variance > 0 else 1.0
    for row in rows:
        z_score = (row.cnt - mean) / std_dev if std_dev > 0 else 0.0
        if abs(z_score) > 1.5:
            session.add(
                AnomalyAlert(
                    DistrictID=row.PoliceStationID,
                    CrimeHeadID=row.CrimeMajorHeadID,
                    WeekStart=now - timedelta(weeks=4),
                    ZScore=round(z_score, 4),
                    AlertLevel=1 if abs(z_score) > 2.0 else 0,
                    ObservedCount=row.cnt,
                    BaselineMean=round(mean, 4),
                    StdDev=round(std_dev, 4),
                )
            )
            print(
                f"  Anomaly alert: district {row.PoliceStationID}, crime head {row.CrimeMajorHeadID}, z={z_score:.2f}"
            )

    # Hotspot layers — one per district with case density
    for district_id in range(1, 32):
        ...
        if case_count > 0:
            density = min(1.0, case_count / 10.0)
            session.add(
                HotspotLayer(
    ...
    await session.commit()
    print("Pre-computed risk scores, anomaly alerts, and hotspot layers.")
...
async def main(db_url: str):
    ...
    async with async_session_local() as session:
        ...
        await seed_compute(session)

    await engine.dispose()
    print("\nSeed complete. 24 cases, 7 entities, risk/anomaly/hotspot pre-computed.")
