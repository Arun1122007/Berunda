...
class MOSimilarityService(BaseService):
    ...
    async def find_similar_cases(
        ...
        stmt = (
            select(CaseMaster, InvOccuranceTime)
            .outerjoin(InvOccuranceTime, CaseMaster.CaseMasterID == InvOccuranceTime.CaseMasterID)
                ...
                results.append(
                    {
                        ...
                        "briefFactsSnippet": (cand_occ.BriefFacts[:120] + "...")
                        if len(cand_occ.BriefFacts) > 120
                        else cand_occ.BriefFacts,
                        "similarityScore": round(score, 4),
                        "matchType": "SERIAL_MO_PATTERN"
                        if score > 0.65
                        else "SIMILAR_CIRCUMSTANCES",
                    }
                )
